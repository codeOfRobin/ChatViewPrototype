...
class ChatTableManager: NSObject, ASTableDataSource, ASTableDelegate, ASEditableTextNodeDelegate {
	...
	func tableNode(_ tableNode: ASTableNode, nodeForRowAt indexPath: IndexPath) -> ASCellNode {
		return {
			if indexPath.row == 3 {
				...
				textNode.attributedPlaceholderText = NSAttributedString(string: "PLACEHOLDER")
				textNode.style.preferredSize = CGSize(width: 100, height: 100)
				textNode.delegate = self
				node.layoutSpecBlock = {
					_ in
					ASStackLayoutSpec(direction: .horizontal, spacing: 0.0, justifyContent: .center, alignItems: .stretch, children: [textNode])
				}
